...
class PengajarControllerUpdate extends REST_Controller
{
    ...
    public function updatePengajar_put($id)
    {
        ...
        $update_result = $pengajar->update_Pengajar($id, $data);

        if ($update_result > 0) {
            $this->response([
                'status' => true,
                'message' => 'NEW Pengajar Updated'
            ], REST_Controller::HTTP_OK);
        } else {
            $this->response([
                'status' => false,
                'message' => 'FAILDE TO Updated Pengajar'
